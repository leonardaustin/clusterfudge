...
import { AlertTriangle, X, RefreshCw, ArrowUp, Check, Copy } from 'lucide-react'
...
function UpdateBanner() {
  ...
  const [dismissed, setDismissed] = useState(updateBannerDismissed)
      ...
      message={
        upgradeCommand ? (
          <span className="flex items-center gap-2">
            ...
            <button
              onClick={copyCommand}
              className="inline-flex items-center gap-1 font-mono text-xs bg-bg-hover px-1.5 py-0.5 rounded hover:bg-bg-tertiary transition-colors"
              title={copied ? "Copied!" : "Copy upgrade command"}
            >
              {upgradeCommand}
