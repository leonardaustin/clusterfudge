...
export function SecretWizard() {
  ...
  const buildSpec = () => {
    ...
    return { name, namespace, type: 'Opaque', data }
  }
  ...
  const goNext = async () => {
    ...
    if (nextStep === 'preview') {
      try {
        const yaml = await PreviewSecret(buildSpec())
        setPreview(yaml)
        setPreviewError('')
      } catch (e) {
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      await ApplyResource('', 'v1', 'secrets', namespace, bytes)
      useToastStore.getState().addToast({ type: 'success', title: `Secret "${name}" created` })
    } catch (e) {
  ...
  return (
    <div className="resource-view">
      <ResourceHeader title="Secret Wizard" subtitle="Create a new Secret step by step" />
        ...
        {step === 'basic' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <div>
              ...
              <input className="settings-input" value={name} onChange={(e) => setName(e.target.value)} style={{ width: '300px' }} placeholder="my-secret" />
            </div>
            <div>
        ...
        {step === 'data' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            <div style={{ fontSize: 'var(--text-xs)', color: 'var(--text-secondary)', marginBottom: '4px' }}>
              Key-value data entries (values are stored as stringData - plain text)
            </div>
