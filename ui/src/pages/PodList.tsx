...
import { ScrollText, TerminalSquare, Cable, Sparkles, Loader2 } from 'lucide-react'
...
import { ListPortForwards, StopPortForward, type PortForwardInfo } from '../wailsjs/go/handlers/StreamHandler'
...
export function PodList() {
  ...
  const [forwardedPods, setForwardedPods] = useState<Map<string, PortForwardInfo[]>>(new Map())
  const [stopForwardTarget, setStopForwardTarget] = useState<PortForwardInfo[] | null>(null)
  const pendingForwardPod = useRef<string | null>(null)
  ...
  const refreshForwards = useCallback(() => {
    ListPortForwards()
      .then((fwds) => {
        const newMap = new Map<string, PortForwardInfo[]>()
        for (const f of fwds) {
          const key = `${f.namespace}/${f.podName}`
          const existing = newMap.get(key) || []
          existing.push(f)
          newMap.set(key, existing)
        }
        setForwardedPods(newMap)
        if (pendingForwardPod.current && newMap.has(pendingForwardPod.current)) {
          pendingForwardPod.current = null
        }
      })
      .catch(() => {})
  }, [])
  ...
  useEffect(() => {
    refreshForwards()
    const interval = setInterval(refreshForwards, 5000)
  ...
  // Extract container ports from a pod's raw spec
  ...
  const openPortForward = useCallback((podName: string, podNamespace: string) => {
    const podItem = items.find((i) => i.name === podName && i.namespace === podNamespace)
  ...
  // Vim j/k/h/l navigation
  useVimNavigation(
    filtered,
    selectedName ? `${selectedNamespace}/${selectedName}` : undefined,
    (p) => `${p.namespace}/${p.name}`,
    (p) => `/workloads/pods/${p.namespace}/${p.name}`,
    '/workloads/pods'
  )

  // Keyboard shortcut: F to open port forward dialog for selected pod
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      if (e.key === 'f' && !e.ctrlKey && !e.metaKey && !e.altKey) {
        ...
        if (selectedName) {
          e.preventDefault()
          openPortForward(selectedName, detailNamespace)
        }
      }
    }
    window.addEventListener('keydown', handler)
    return () => window.removeEventListener('keydown', handler)
  }, [selectedName, detailNamespace, openPortForward])
    ...
    <div className="resource-view">
                ...
                <div className="detail-panel-body">
                  {activeTab === 'Overview' && (
                    <>
                      ...
                      <Tooltip.Provider delayDuration={300}>
                        <div className="flex items-center gap-1.5" style={{ marginBottom: 'var(--space-4)' }}>
                          ...
                          <Tooltip.Root>
                            ...
                            <Tooltip.Portal>
                              <Tooltip.Content side="bottom" sideOffset={4} className="bg-bg-tertiary border border-border text-text-primary text-xs px-2 py-1 rounded-md shadow-lg z-50">
                                View Logs
                          ...
                          {(() => {
                            const podKey = `${detailNamespace}/${selectedName}`
                            const isActive = forwardedPods.has(podKey)
                            const isPending = pendingForwardPod.current === podKey && !isActive
                            return (
                              <Tooltip.Root>
                                <Tooltip.Trigger asChild>
                                  <button
                                    onClick={() => {
                                      ...
                                      if (isActive) {
                                        setStopForwardTarget(forwardedPods.get(podKey) || null)
                                      } else {
                                        openPortForward(selectedName, detailNamespace)
                                      }
                                    }}
                                ...
                                <Tooltip.Portal>
                                  <Tooltip.Content side="bottom" sideOffset={4} className="bg-bg-tertiary border border-border text-text-primary text-xs px-2 py-1 rounded-md shadow-lg z-50">
                                    {isPending ? 'Connecting...' : isActive ? 'Port Forward Active' : 'Port Forward'}
                                    <Tooltip.Arrow className="fill-border" />
                                  </Tooltip.Content>
                                </Tooltip.Portal>
                              </Tooltip.Root>
                  ...
                  {activeTab === 'YAML' && detailRaw && (
                    <div className="log-viewer" style={{ maxHeight: '500px' }}>
                      <CodeBlock code={JSON.stringify(detailRaw, null, 2)} language="json" />
                    </div>
                  )}
                </div>
              </DetailPanel>
      ...
      {portForwardTarget && (
        <PortForwardDialog
          ...
          onOpenChange={(open) => {
            if (!open) {
              // Mark as pending so the icon shows a spinner until the forward appears
              pendingForwardPod.current = `${portForwardTarget.namespace}/${portForwardTarget.name}`
              setPortForwardTarget(null)
              // Quick poll to pick up the new forward faster
              setTimeout(refreshForwards, 500)
              setTimeout(refreshForwards, 1500)
            }
          }}
      ...
      {/* Stop port forward confirmation */}
      <Dialog.Root open={stopForwardTarget !== null} onOpenChange={(open) => { if (!open) setStopForwardTarget(null) }}>
        <Dialog.Portal>
          <Dialog.Overlay className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50" />
          <Dialog.Content className="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-bg-secondary border border-border rounded-xl shadow-2xl p-5 w-[400px] z-50">
            <Dialog.Title className="text-sm font-semibold text-text-primary mb-3">
              Stop Port Forward{stopForwardTarget && stopForwardTarget.length > 1 ? 's' : ''}
            </Dialog.Title>
            <div className="space-y-2 mb-4">
              {stopForwardTarget?.map((f) => (
                <div key={f.localPort} className="flex items-center gap-2 text-xs text-text-secondary bg-bg-tertiary rounded px-3 py-2">
                  <Cable className="w-3.5 h-3.5 text-accent shrink-0" />
                  <span className="font-mono">localhost:{f.localPort}</span>
                  <span className="text-text-tertiary">&rarr;</span>
                  <span className="font-mono">{f.podName}:{f.podPort}</span>
                </div>
              ))}
            </div>
            <div className="flex justify-end gap-2">
              <button
                onClick={() => setStopForwardTarget(null)}
                className="px-3 py-1.5 text-xs text-text-secondary hover:text-text-primary rounded border border-border hover:bg-bg-hover transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={async () => {
                  if (!stopForwardTarget) return
                  for (const f of stopForwardTarget) {
                    await StopPortForward(f.localPort).catch(() => {})
                  }
                  setStopForwardTarget(null)
                  refreshForwards()
                }}
