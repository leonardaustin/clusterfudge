...
import (
	...
	"clusterfudge/internal/portforward"
...
// StopAllPortForwards stops all active port forwards and cancels their contexts.
func (h *StreamHandler) StopAllPortForwards() {
	h.pfManager.StopAllPortForwards()

	h.mu.Lock()
	cancels := make([]context.CancelFunc, 0, len(h.pfCancels))
	for port, cancel := range h.pfCancels {
		cancels = append(cancels, cancel)
		delete(h.pfCancels, port)
	}
	h.mu.Unlock()
...
// OnSystemWake is called when the OS wakes from sleep. It probes all
// active port forwards and triggers reconnection for dead ones.
func (h *StreamHandler) OnSystemWake() {
	cs, err := h.manager.ActiveClient()
	if err != nil {
		// No active cluster connection; nothing to probe.
		return
	}
	h.pfManager.ProbeAll(context.Background(), cs.Typed)
}

// DiscoverPortForwards scans services for port-forward annotations.
func (h *StreamHandler) DiscoverPortForwards(namespace string) ([]portforward.DiscoveredForward, error) {
	cs, err := h.manager.ActiveClient()
	if err != nil {
		return nil, fmt.Errorf("discover port forwards: %w", err)
	}
	return portforward.DiscoverPortForwards(context.Background(), cs.Typed, namespace)
}
