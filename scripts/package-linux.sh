...
# Package KubeViewer as an AppImage for Linux distribution
#
# Prerequisites:
#   - appimagetool (https://github.com/AppImage/AppImageKit)
#   - A built binary at build/bin/kubeviewer

APP_NAME="KubeViewer"
APP_ID="com.leonardaustin.kubeviewer"
...
# Copy binary
cp "$BINARY" "${APP_DIR}/usr/bin/kubeviewer"
chmod +x "${APP_DIR}/usr/bin/kubeviewer"
...
SELF=$(readlink -f "$0")
HERE=${SELF%/*}
exec "${HERE}/usr/bin/kubeviewer" "$@"
APPRUN
chmod +x "${APP_DIR}/AppRun"

# Copy icon if available, otherwise create placeholder
if [ -f "build/appicon.png" ]; then
    cp "build/appicon.png" "${APP_DIR}/usr/share/icons/hicolor/256x256/apps/kubeviewer.png"
    ln -sf "usr/share/icons/hicolor/256x256/apps/kubeviewer.png" "${APP_DIR}/kubeviewer.png"
else
