...
// Search searches across multiple GVRs for resources whose name contains
// the query string (case-insensitive). It searches concurrently and caps
// results at maxResults.
func (s *Service) Search(ctx context.Context, client dynamic.Interface, gvrs []schema.GroupVersionResource, query string, maxResults int) []SearchResult {
	if len(query) < 2 || maxResults <= 0 {
		return nil
	}

	queryLower := strings.ToLower(query)

	type gvrResult struct {
		items []SearchResult
	}

	const workers = 8
	jobs := make(chan schema.GroupVersionResource, len(gvrs))
	resultsCh := make(chan gvrResult, len(gvrs))
			...
			for gvr := range jobs {
				...
				for _, item := range items {
					...
					matched = append(matched, SearchResult{
						...
						GVR:       gvr.Group + "/" + gvr.Version + "/" + gvr.Resource,
					})
				}
				if len(matched) > 0 {
					resultsCh <- gvrResult{items: matched}
				}
			}
		}()
	}
