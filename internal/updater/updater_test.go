...
func TestParseVersion(t *testing.T) {
	tests := []struct {
		...
		{"2026.0320.1924", [3]int{2026, 320, 1924}},
		{"2026.0320.1344", [3]int{2026, 320, 1344}},
		{"2026.0319.1825", [3]int{2026, 319, 1825}},
		{"2026.0101.0000", [3]int{2026, 101, 0}},
		{"2026.1231.2359", [3]int{2026, 1231, 2359}},
		// CalVer with git describe suffix (dirty builds)
		{"2026.0320.1924-3-g8a85a6b", [3]int{2026, 320, 1924}},
		{"2026.0320.1924-dirty", [3]int{2026, 320, 1924}},
		// Semver (backwards compat)
		{"1.2.3", [3]int{1, 2, 3}},
...
func TestIsNewer(t *testing.T) {
	tests := []struct {
		...
		// CalVer: same day, newer time
		{"calver same day newer time", "2026.0320.1924", "2026.0320.1344", true},
		// CalVer: newer day
		{"calver newer day", "2026.0321.0800", "2026.0320.1924", true},
		// CalVer: same version
		{"calver same version", "2026.0320.1344", "2026.0320.1344", false},
		// CalVer: older version
		{"calver older version", "2026.0320.1344", "2026.0320.1924", false},
		// CalVer: different year
		{"calver newer year", "2027.0101.0000", "2026.1231.2359", true},
		// CalVer with v prefix stripped (matches real usage)
		{"calver v-stripped", "2026.0320.1924", "2026.0320.1344", true},
		// Semver (backwards compat)
		{"semver minor bump", "1.1.0", "1.0.0", true},
		{"semver patch bump", "1.0.1", "1.0.0", true},
		{"semver major bump", "2.0.0", "1.9.9", true},
		{"semver same", "1.0.0", "1.0.0", false},
		{"semver older", "0.9.0", "1.0.0", false},
	}
	for _, tt := range tests {
...
func TestCheckForUpdate_CalVer(t *testing.T) {
	releaseJSON := `{
		"tag_name": "v2026.0320.1924",
		"html_url": "https://github.com/test/test/releases/v2026.0320.1924",
		"body": "Release notes",
		"draft": false,
		"prerelease": false,
		"published_at": "2026-03-20T19:38:30Z",
		"assets": [
			{"name": "clusterfudge_darwin_arm64.dmg", "browser_download_url": "ASSET_URL/clusterfudge_darwin_arm64.dmg", "size": 1000},
			{"name": "clusterfudge_darwin_amd64.dmg", "browser_download_url": "ASSET_URL/clusterfudge_darwin_amd64.dmg", "size": 1000},
			{"name": "clusterfudge_linux_amd64.tar.gz", "browser_download_url": "ASSET_URL/clusterfudge_linux_amd64.tar.gz", "size": 1000},
			{"name": "clusterfudge_linux_arm64.tar.gz", "browser_download_url": "ASSET_URL/clusterfudge_linux_arm64.tar.gz", "size": 1000}
		]
	}`

	var srvURL string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		t.Logf("mock server got request: %s %s", r.Method, r.URL.Path)
		if r.URL.Path == "/repos/test/test/releases/latest" {
			w.Header().Set("Content-Type", "application/json")
			body := strings.ReplaceAll(releaseJSON, "ASSET_URL", srvURL)
			w.Write([]byte(body))
			return
		}
	...
	tests := []struct {
		...
		{"older CalVer detects update", "2026.0320.1344", true},
		{"same CalVer no update", "2026.0320.1924", false},
		{"newer CalVer no update", "2026.0321.0800", false},
		{"older day detects update", "2026.0319.1825", true},
		{"v-prefixed current still works", "v2026.0320.1344", true},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			u := NewUpdater("test", "test", tt.current)
			u.baseURL = srv.URL
			...
			if tt.wantUpdate && info == nil {
				t.Error("expected update info, got nil")
			}
			if !tt.wantUpdate && info != nil {
				t.Errorf("expected no update, got version %s", info.Version)
			}
			if tt.wantUpdate && info != nil {
				if info.Version != "v2026.0320.1924" {
					t.Errorf("expected version v2026.0320.1924, got %s", info.Version)
				}
			}
		})
	}
...
func TestCheckForUpdate_DraftRelease(t *testing.T) {
	releaseJSON := `{
		"tag_name": "v2026.0320.1924",
		"html_url": "https://github.com/test/test/releases/v2026.0320.1924",
		"body": "Release notes",
		...
		"published_at": "2026-03-20T19:38:30Z",
		"assets": [
			{"name": "clusterfudge_darwin_arm64.dmg", "browser_download_url": "http://example.com/clusterfudge_darwin_arm64.dmg", "size": 1000}
		]
	}`
	...
	u := NewUpdater("test", "test", "2026.0320.1344")
	u.baseURL = srv.URL
	info, err := u.CheckForUpdate(context.Background())
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	if info != nil {
		t.Error("expected nil for draft release, got update info")
	}
}
