...
func (u *Updater) CheckForUpdate(ctx context.Context) (*UpdateInfo, error) {
	...
	if release.Draft || release.Prerelease {
		log.Printf("updater: skipping draft/prerelease %s", release.TagName)
		return nil, nil
	}
	...
	if asset == nil {
		log.Printf("updater: no asset matching %q in release %s", assetName, release.TagName)
		return nil, nil
	}

	checksum := u.findChecksum(ctx, release.Assets, assetName)

	log.Printf("updater: update available %s -> %s", u.current, release.TagName)
	return &UpdateInfo{
