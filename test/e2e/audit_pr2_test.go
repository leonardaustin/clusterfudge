...
// TestPerOperationTimeout_GetClusterSummaryCompletesQuickly verifies that
// per-operation timeouts inside GetClusterSummary do not interfere with
// normal operation. Unlike TestGetClusterSummary_PaginatedCounts (which
// tests count accuracy), this test focuses on latency.
func TestPerOperationTimeout_GetClusterSummaryCompletesQuickly(t *testing.T) {
	mgr := newClusterManager(t)
	ch := handlers.NewClusterHandler(mgr)
	...
	// Per-operation timeout is 10s each; total should be well under 30s on a healthy cluster.
	if elapsed > 30*time.Second {
		t.Errorf("GetClusterSummary took %v — unexpectedly slow, possible timeout issue", elapsed)
	}
	t.Logf("GetClusterSummary completed in %v", elapsed)
}
...
func TestWatchKeyDuplicateReplace(t *testing.T) {
	...
	// Create a configmap — should only produce one ADDED event from the
	// active (second) watch, not two (which would mean the first watch
	// was not replaced). We verify this indirectly: if StopWatch cleanly
	// stops and a subsequent StopWatch is a no-op, there is exactly one
	// active watch — confirming the first was replaced.
	h.StopWatch("", "v1", "configmaps", testEnv.namespace)

	// A subsequent StopWatch on the same key should be a no-op (no panic).
	h.StopWatch("", "v1", "configmaps", testEnv.namespace)

	// After stopping, a new WatchResources should succeed — confirming
	// the key was fully cleaned up.
	if err := h.WatchResources(ctx, "", "v1", "configmaps", testEnv.namespace); err != nil {
		t.Fatalf("WatchResources after stop should succeed: %v", err)
	}
	h.StopWatch("", "v1", "configmaps", testEnv.namespace)
}
...
// TestDrainNode_CordonsAndCompletes verifies that DrainNode cordons the node
// and runs to completion without panicking. We test on an actual node but
// immediately uncordon it to restore cluster state.
func TestDrainNode_CordonsAndCompletes(t *testing.T) {
	mgr := newClusterManager(t)
	h := handlers.NewResourceHandler(resource.NewService(), mgr)
