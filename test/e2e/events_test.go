...
func TestEvents_ConcurrentEmitSubscribe(t *testing.T) {
	...
	// Keep a long-lived subscriber to count total received events.
		...
		go func() {
			...
			for j := 0; j < 100; j++ {
				...
				case <-ctx.Done():
					return
				default:
					u := em.Subscribe("concurrent", func(payload any) {})
					u()
				}
			}
		}()
	...
	// The long-lived subscriber should have received some events.
	if got := receivedCount.Load(); got == 0 {
		t.Error("long-lived subscriber received 0 events; expected > 0")
	}
}
