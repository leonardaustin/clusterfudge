...
func TestLogs_LinesInOrder(t *testing.T) {
	...
	// Lines should contain "line-N" with monotonically increasing N.
	prevNum := -1
	for i, line := range collected {
		if !strings.Contains(line, "line-") {
			t.Errorf("line %d doesn't contain expected prefix: %q", i, line)
			continue
		}
		// Extract the integer suffix after "line-".
		idx := strings.LastIndex(line, "line-")
		numStr := strings.TrimSpace(line[idx+len("line-"):])
		num := 0
		...
		if prevNum >= 0 && num <= prevNum {
			t.Errorf("lines out of order: line %d has counter %d, previous had %d", i, num, prevNum)
		}
		prevNum = num
	}
}
...
func TestLogs_TailLines(t *testing.T) {
	...
	if count == 0 {
		t.Fatal("expected tail lines, got 0")
	}
	if count > 20 {
		t.Errorf("expected <= 20 lines with TailLines=20, got %d", count)
	}

	// Verify the last line received is "line 50" (the tail end of output).
	mu.Lock()
	lastLine := lines[len(lines)-1].Content
	mu.Unlock()
	if !strings.Contains(lastLine, "line 50") {
		t.Errorf("expected last tail line to contain 'line 50', got %q", lastLine)
	}
}
...
func TestLogs_StopStream(t *testing.T) {
	...
	// Wait for at least 1 line to arrive (event-based, not time-based).
	...
	// After the stream goroutine has exited, no new lines should arrive.
	mu.Lock()
