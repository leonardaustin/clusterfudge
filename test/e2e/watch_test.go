...
func TestWatch_Reconnect(t *testing.T) {
	...
	// Drain remaining events and verify channel closes.
	channelClosed := false
	...
	for !channelClosed {
		...
		case <-drainDeadline.C:
			t.Log("watch channel did not close within 5s after cancel (may have buffered events)")
			channelClosed = true // proceed to reconnect test
		}
	}
