...
func TestErrors_RBACForbidden(t *testing.T) {
	...
	// Try operations that should be forbidden for an SA with no role bindings.
	// k3s may allow listing some resources by default, so we try multiple
	// operations and require at least one to return Forbidden.
	forbiddenOps := []struct {
		name string
		fn   func() error
	}{
		{"list secrets in kube-system", func() error {
			_, err := restrictedCS.Typed.CoreV1().Secrets("kube-system").List(ctx, metav1.ListOptions{})
			return err
		}},
		{"delete pod in kube-system", func() error {
			return restrictedCS.Typed.CoreV1().Pods("kube-system").Delete(ctx, "nonexistent-e2e", metav1.DeleteOptions{})
		}},
		{"list nodes", func() error {
			_, err := restrictedCS.Typed.CoreV1().Nodes().List(ctx, metav1.ListOptions{})
			return err
		}},
	}
	...
	if !gotForbidden {
		t.Error("expected at least one operation to return Forbidden for unprivileged SA")
	}
}
