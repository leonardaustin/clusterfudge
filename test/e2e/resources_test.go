...
func TestListPVCsAndPVs(t *testing.T) {
	...
	// Look up the specific PV that the PVC bound to.
	pvcObj, pvcErr := testEnv.typed.CoreV1().PersistentVolumeClaims(testEnv.namespace).Get(ctx, pvcName, metav1.GetOptions{})
	if pvcErr != nil {
		t.Fatalf("get pvc for volume name: %v", pvcErr)
	}
	boundPVName := pvcObj.Spec.VolumeName
	if boundPVName == "" {
		t.Fatal("PVC is bound but has empty VolumeName")
	}
	...
	if findByName(pvs, boundPVName) == nil {
		t.Errorf("expected PV %q (bound to PVC %q) in PV list, not found among %d PVs", boundPVName, pvcName, len(pvs))
	}
}
...
func TestListPods_EmptyNamespace(t *testing.T) {
	...
	if len(items) != 0 {
		t.Errorf("expected 0 pods in fresh namespace, got %d", len(items))
	}
}
