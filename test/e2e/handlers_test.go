...
func TestHandlerCluster_ListContexts(t *testing.T) {
	...
	// Verify the current context is in the returned list.
	currentCtx := newContextName(t)
	found := false
	for _, c := range contexts {
		if c.Name == currentCtx {
			found = true
			break
		}
	}
	if !found {
		t.Errorf("current context %q not found in ListContexts result (%d contexts)", currentCtx, len(contexts))
	}
}
...
func TestHandlerHelm_ListReleases(t *testing.T) {
	...
	// Fresh namespace should have zero releases — verify the result
	// is a valid (possibly empty) slice, not nil from an error path.
	if releases == nil {
		t.Error("expected non-nil releases slice (empty is ok), got nil")
	}
}
